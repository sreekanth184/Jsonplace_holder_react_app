...
function User_page() {
    ...
    if(input=="Album"){
        output= album.length? album.map((userAl)=><Album_layout  key={userAl.id} userAl={userAl}/>):"Loading..."
    }else if(input=="Todos"){
        output= todo.length? todo.map((usertodo)=><Todo_layout  key={usertodo.id} usertodo={usertodo}/>):"Loading..."
    }else if(input=="posts"){
        output= posts.length? posts.map((userposts)=><Post_layout key={userposts.id} userposts={userposts}/>):"Loading..."
    }else{
